...
def main():
    ...
    results_folder = PATHS['RESULTS'] / 'units_place_wzscore'
    reports_folder = PATHS['REPORTS'] / 'units_place_wzscore'
    ...
    for nwbfilename in nwbfiles:
        ...
        for unit_ind in range(len(nwbfile.units)):
            ...
            try:
                ...
                # Zscore
                z_scores = zscore(trial_place_bins, nan_policy='omit',axis=None)
                trial_place_bins[z_scores > EXCLUSION['zscore']] = np.nan
                
                z_score_trial_place_bins = z_scores
                z_score_trial_place_bins[z_scores >EXCLUSION['zscore']]=np.nan
                ...
                if 'Z_SCORE' in METHODS['PLACE']:
                    results['place_z_score'] = fit_anova_place(create_df_place(z_score_trial_place_bins))
...
###################################################################################
                ...
                for ind, shuffle in enumerate(shuffles):
                    ...
                    surr_z_scores = zscore(surr_trial_place_bins, nan_policy = 'omit',axis=None)
                    outlier_threshold = EXCLUSION['zscore']
                    surr_trial_place_bins[z_scores > outlier_threshold] = np.nan
                    
                    surr_z_score_trial_place_bins = surr_z_scores
                    surr_z_score_trial_place_bins[surr_z_scores > outlier_threshold]=np.nan
                    ...
                    if 'Z_SCORE' in METHODS['PLACE']:
                        surrs['place_z_score'][ind] = fit_anova_place(create_df_place(surr_z_score_trial_place_bins))
...
#                 ## MAKE REPORT
                ...
                P = results['place_z_score_surr_p_val']
                ZF = results['place_z_score']
                plot_surrogates(surrs['place_anova'], data_value=ZF, p_value=None ,title=f'P = {P}',
                                                     title_color=color_pvalue(P),ax = ax,alpha = .6,color = 'grey')
                add_vlines(ZF , ax, color='darkred', linestyle='solid', linewidth=4)
                ax.plot(ZF , 0, 'o', zorder=10, clip_on=False, color='darkred', markersize=10)
                ax.set_xlabel('Z-scored F - Statistics')
                ax.set_ylabel('count')
                drop_spines(['top', 'right'],ax)
